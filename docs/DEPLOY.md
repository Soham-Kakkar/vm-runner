...
Use `docs/ISO_TO_QCOW2.md` for the exact ISO-to-working-CTF checklist. The
short version is:

1. Install the ISO into a reusable qcow2 disk.
2. Boot the installed disk with `--share guest-share`.
3. Inside the guest, install the `ctf` user, `vmrunner` user, helper binary,
   `place_flags.sh`, and boot service.
4. Edit and test `place_flags.sh` inside the guest so it creates the expected
   challenge files.
5. Shut down the guest cleanly.
6. Upload the qcow2 in the web maker.
7. Enter only the question number, name, text, validation style, and flag
   format in the maker UI.
...
The validation format entered in the maker UI must exactly match the guest
script. For example, `flag{text+<hmac>_txt}` and
`flag{txt+<hmac>_txt}` are different flags.
